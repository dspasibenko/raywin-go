...
func (ow *ObjectsWriter) WritePureString(v string) (int, error) {
	return ow.WritePureBytes(cast.StringToByteArray(v))
}
...
func (ow *ObjectsWriter) WriteString(v string) (int, error) {
	return ow.WriteBytes(cast.StringToByteArray(v))
}
...
func MarshalString(v string, buf []byte) (int, error) {
	return MarshalBytes(cast.StringToByteArray(v), buf)
}
...
func UnmarshalString(buf []byte, newBuf bool) (int, string, error) {
	...
	if err == nil {
		return idx, cast.ByteArrayToString(res), err
	}
	return idx, "", err
}
...
func WritableStringSize(v string) int {
	return WritebleBytesSize(cast.StringToByteArray(v))
}
